...
def build_search_index(model_path, index_dataset, mode):
    ...
    if mode == 'ft':
        ...
        index_out['BodyVectors'] = build_ft_vecs(ft_model,
                                                 list(index_dataset['Body']))
        index_out['TitleVectors'] = build_ft_vecs(ft_model,
                                                  list(index_dataset['Title']))
        index_out['TagVectors'] = build_ft_vecs(ft_model,
                                                tags_to_text_list(
                                                    index_dataset['Tags']))
        return index_out
    elif mode == 'tfidf':
        ...
        index_out['BodyVectors'] = build_tfidf_vecs(tfidf_model,
                                                    list(
                                                        index_dataset['Body']))
        index_out['TitleVectors'] = build_tfidf_vecs(
            tfidf_model, list(index_dataset['Title']))
        index_out['TagVectors'] = build_tfidf_vecs(tfidf_model,
                                                   tags_to_text_list(
                                                       index_dataset['Tags']))
        return index_out
    else:
...
def build_metadata_index(src_database_path, qids, query, export_path):
    ...
    for row in c:
        str_out = {
            ...
            'Score': row[1],
            'Title': row[2],
            'Tags': split_tags(row[3]),
            'SnippetCount': row[4],
            'Snippets': row[5].split('<pdelim>')
        }
...
def main(temp_dir, index_export_dir, src_database_path, src_question_dataframe,
         tfidf_model_path, fasttext_model_path, index_qids_query,
         metadata_query):
    # Paths
    ...
    dataframe_export_path = os.path.join(index_export_dir, 'data',
                                         'index_dataset.pkl')
    ...
    build_metadata_index(src_database_path, index_ids, metadata_query,
                         metadata)
    ...
    process_corpus(
        bodies_corpus, processed_bodies_corpus, del_stack_trace=True)
    dump_text_list(titles_corpus, index_dataset['Title'])
    process_corpus(
        titles_corpus, processed_titles_corpus, del_stack_trace=False)
    ...
    print(
        len(index_ids), len(index_dataset['Body']), len(
            index_dataset['Title']))
    # Output dataframe to export folder
    pd.DataFrame(
        index_dataset, index=index_ids).to_pickle(dataframe_export_path)
    index_ids = None
    ...
    with open(ft_postvec_index, 'wb') as out:
        pickle.dump(
            build_search_index(fasttext_model_path, index_dataset, 'ft'), out)
    ...
    with open(tfidf_doc_vec_index, 'wb') as out:
        pickle.dump(
            build_search_index(tfidf_model_path, index_dataset, 'tfidf'), out)
