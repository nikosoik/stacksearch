...
ce_features = os.path.join(file_path, 'data/ce_features/ce_feat_min3.json')
ft_model = os.path.join(file_path, 'data/fasttext/fasttext_v0.1.bin')
ft_cluster_dir = os.path.join(file_path, 'data/mbkm_clusters')
brown_paths = os.path.join(file_path, 'data/brown/paths.pkl')
gazetteer_dir = os.path.join(file_path, 'data/gazetteers')


class FeatureExtractor:
    def __init__(self,
                 use_models=False,
                 ce_features=ce_features,
                 ft_model=ft_model,
        ...
        if use_models:
            print('Using fastText and MiniBatchKMeans models.')
            self.use_models = True
            self.fasttext = load_model(ft_model)
            self._load_clusterers(ft_cluster_dir)
        else:
            print('Using pre-calculated compound embedding features.')
            self.use_models = False
            self._read_ce_features(ce_features)

        self.brown_paths = pd.read_pickle(brown_paths)
        self._load_gazetteers(gazetteer_dir)
        self._build_template()

    def _read_ce_features(self, filename):
        with open(filename, 'r') as _in:
            self.ce_features = json.load(_in)
            self.ce_names = self.ce_features['__CLUSTER_NAMES__']
            self.ce_defval = [''] * len(self.ce_names)
    ...
    def _compound_embedding_features(self, token):
        if self.use_models:
            ...
            token_ces = self.ce_features.get(token, self.ce_defval)
            for ii, name in enumerate(self.ce_names):
                self.feat['ce' + name] = token_ces[ii]
