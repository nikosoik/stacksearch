...
from .wordvec_model.search_model import BaseSearchModel
...
class FastTextSearch(BaseSearchModel):
    def __init__(self, model_path, index_path, index_keys, metadata_path,
                 wordvec_index_path=None, api_dict_path=None):
    ...
    def infer_vector(self, text):
        return self.model.get_sentence_vector(text.lower().strip()).reshape(1, -1)
    ...
    def print_api_labels(self, query_vec, search_depth=1400, max_n=10):
        ...
        """
        sims = self._calc_cossims(query_vec, self.wv_index.values, batch_calc=False)
    ...
    def search(self, num_results=20, custom_weights=None, postid_fn=None,
        api_labels=False, vector_fn_kwargs={}):
