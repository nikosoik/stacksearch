...
def process_corpus(corpus_path, export_file, del_long_strings):
    # Bash Scripts / Paths
    script_1 = os.path.join(script_path, 'del_long_strings.sh')
    ...
    if del_long_strings:
        ...
        subprocess.check_call([script_1, corpus_path])
        corpus_path = corpus_path + '_nst'
    # Load the custom spaCy tokenizer
    nlp = get_custom_tokenizer()
    print('spaCy tokenizer loaded...\nProcessing text...')
    unigrams_to_disk(normalized_line_feed, corpus_path, export_file, nlp)
    # Normalize corpus
    print('Normalizing corpus (removing unneeded punctuation and strings)...')
    export_file = os.path.realpath(export_file)
    subprocess.check_call([script_2, export_file])
    print('Output corpus at:', export_file + '_norm')


if __name__ == '__main__':
    TEST = 'input.test'
    process_corpus(TEST, os.path.join(os.path.curdir, 'output.test'), True)
    """
