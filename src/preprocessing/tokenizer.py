...
def add_custom_properties(nlp):
    ...
    def is_symbol_getter(token): return (
        len(token) == 1 and unicodedata.category(token.text).startswith('S'))
...
if __name__ == '__main__':
    ...
    doc = nlp(result)
    result = u' '.join(t.norm_ for t in doc if not (
        t.is_punct or t.is_bracket or t.is_quote or t._.is_symbol or t.like_num))
    print(result, end='\n\n')
